...
engine({
    ...
    root: {
        ...
        update(node) {
            const PI = Math.PI;
            const t = new Date(), _secs = t.getSeconds() + t.getMilliseconds() / 1000, _mins = t.getMinutes() + _secs / 60, _hours = t.getHours() + _mins / 60;
            node.children.hours.box.rotation = PI + (PI / 6) * _hours;
            node.children.minutes.box.rotation = PI + (PI / 30) * _mins;
            node.children.seconds.box.rotation = PI + (PI / 30) * _secs;
        },
    },
});
